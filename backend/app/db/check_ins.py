...
statement — the pooler role bypasses RLS, so that filter is the only thing isolating one
user's data from another's (see app/auth.py and .claude/rules/backend.md). Someone else's
row simply matches nothing: a 404, never a 403.
"""
